...
class DefaultTest(unittest.TestCase):
    ...
    def test_call_default_for_wrong_dict_cmd(self):
        with patch.object(Jarvis.Jarvis, 'default') as default_mock:
            Jarvis.Jarvis()._generate_output_if_dict(self.action, "enable", ["cat", "and", "dog"])
            self.assertTrue(default_mock.called)

    def test_dont_call_default_for_correct_dict_cmd(self):
        with patch.object(Jarvis.Jarvis, 'default') as default_mock:
            Jarvis.Jarvis()._generate_output_if_dict(self.action, "enable", ["your", "sound", "please"])
    ...
    def test_call_default_for_words_before_1_word_dict_cmd(self):
        ...
        with patch.object(Jarvis.Jarvis, '_generate_output_if_dict') as generate_output_mock:
            Jarvis.Jarvis().precmd("please, could you enable")
        ...
        with patch.object(Jarvis.Jarvis, 'default') as default_mock:
            Jarvis.Jarvis()._generate_output_if_dict(*args)
            self.assertTrue(default_mock.called)
